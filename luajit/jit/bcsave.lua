...
local map_type = {
  raw = "raw", c = "c", h = "h", o = "obj", obj = "obj", e = "oe"
}
...
local function bcsave_aes(ctx, input, output) 
	local f = readfile(input) 
	local s = string.dump(f, ctx.strip) 
	-- Add original size as a uint32 to front of data.
	local alldata = aes.encrypt( ctx.modname, s ) 
	bcsave_raw(output, alldata) 
end
